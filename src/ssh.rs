...
impl SshConnection {
    pub fn connect(params: &ConnectionParams) -> Result<Self> {
        ...
        if !session.authenticated() {
            error!("SSH authentication failed for {}@{}", params.username, params.host);
            anyhow::bail!("Authentication failed");
        }
        
        info!("SSH connected successfully to {}@{}", params.username, params.host);
    ...
    fn try_auto_auth(session: &mut Session, username: &str) -> Result<()> {
        ...
        if let Some(auth_sock) = std::env::var_os("SSH_AUTH_SOCK") {
            match session.userauth_agent(username) {
                Ok(()) => return Ok(()),
                Err(e) => {
                    errors.push(format!("SSH agent ({}): {}", auth_sock.to_string_lossy(), e));
                }
            }
        } else {
    ...
    pub fn sftp(&self) -> Result<Sftp> {
        self.session.sftp().context("Failed to create SFTP subsystem")
    }
}
