...
impl EditorManager {
    pub fn new() -> Result<Self> {
        ...
        let remote_files: Arc<Mutex<HashMap<PathBuf, RemoteFileMapping>>> = Arc::new(Mutex::new(HashMap::new()));
        let pending_uploads: Arc<Mutex<Vec<PendingEditorUpload>>> = Arc::new(Mutex::new(Vec::new()));
        ...
        let remote_watcher = Self::setup_remote_watcher(watch_files, watch_pending, tmp_dir.clone())?;
                    ...
                    Ok(event) => {
                        ...
                        if should_check {
                            for path in &event.paths {
                                ...
                                debug!("File event {:?} for path: {:?} (canonical: {:?})", event.kind, path, canonical);
                ...
                match res {
                    Ok(event) => {
                        ...
                        let is_relevant = event.paths.iter().any(|p| {
                            p.parent() == Some(watched_dir.as_path())
                        });
    ...
    pub fn open_local_file(&self, path: &Path, editor_command: &str) -> Result<()> {
        ...
        match result {
            Ok(child) => {
                info!("Successfully spawned editor process (pid: {:?})", child.id());
                Ok(())
            }
            Err(e) => {
                error!("Failed to spawn editor: {} - Error: {}", 
                    if trimmed.is_empty() { "system default" } else { trimmed }, e);
                Err(anyhow::anyhow!("Failed to open editor '{}': {}", 
                    if trimmed.is_empty() { "system default" } else { trimmed }, e))
            }
        }
    }
    ...
    pub fn open_remote_file(
        ...
        
        {
            ...
            info!("Tracking remote file: {:?} -> {}", canonical_tmp_path, remote_path);
